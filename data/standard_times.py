...
STANDARD_TIMES_PATH = Path(__file__).with_name("standard_times.json")

def fetch_standard_times(logger=None):
    data = {
    ...
    STANDARD_TIMES_PATH.write_text(json.dumps(data, ensure_ascii=False, indent=2), encoding="utf-8")
    if logger:
        logger.info("✅ HKJC 標準時間載入：%d 組", len(data))
    return data